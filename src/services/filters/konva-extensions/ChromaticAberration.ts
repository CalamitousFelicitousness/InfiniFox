...
Konva.Filters.ChromaticAberration = function(imageData: ImageData) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      let offsetR = 0, offsetG = 0, offsetB = 0;
      ...
      const samplePixel = (channel: Uint8ClampedArray, xOffset: number, yOffset: number, channelIndex: number) => {
      ...
      const bilinearSample = (channel: Uint8ClampedArray, xOffset: number, yOffset: number, channelIndex: number) => {
    ...
    for (let y = 1; y < height - 1; y++) {
      for (let x = 1; x < width - 1; x++) {
        ...
        if (normalizedDistance > 0.5) {
          ...
          const topValue = data[((y - 1) * width + x) * 4] + 
          ...
          if (edgeStrength > 0.1) {
            ...
            if (dx > 0) {
              data[pixelIndex] = Math.min(255, data[pixelIndex] + fringeStrength * edgeStrength * 30);
              data[pixelIndex + 2] = Math.min(255, data[pixelIndex + 2] + fringeStrength * edgeStrength * 40);
            } else {
              // Add green fringe on the other side
              data[pixelIndex + 1] = Math.min(255, data[pixelIndex + 1] + fringeStrength * edgeStrength * 35);
            }
          }
        }
      }
