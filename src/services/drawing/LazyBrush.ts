...
export class LazyBrush {
  ...
  update(point: Point, friction?: number): boolean {
    ...
    if (this.distance > this.config.radius) {
      ...
      if (this.config.smoothing && this.config.smoothing > 0) {
        ...
        this.brush.x += moveX * (1 - smoothingFactor) + this.getSmoothedVelocity().x * smoothingFactor * deltaTime
        this.brush.y += moveY * (1 - smoothingFactor) + this.getSmoothedVelocity().y * smoothingFactor * deltaTime
      } else {
        this.brush.x += moveX
        this.brush.y += moveY
      }
  ...
  getAdaptiveRadius(): number {
    ...
    const speed = Math.sqrt(
      this.pointer.velocity.x ** 2 + 
      this.pointer.velocity.y ** 2
    )
